...
5. **Rate & Deduction Logic**: Evaluates Grace period rules, calculates `r.deduct`, and handles special overrides (such as `forceOverride` for Cleaners working on Sundays). 
   - **Late Deduction Policies**: The `customRound()` function supports dynamically configurable rounding behaviors from the Master Control UI, including `half` (30-minute blocks), `hour` (60-minute blocks), and `minute` (exact per-minute deduction for precise penalty scaling) policies, while strictly respecting predefined grace periods.
...
- **Contextual Summary Header**: The global header acts as a real-time financial dashboard displaying Actual Labor Cost (Base+OT-Late), Paid Advances, and Remaining Payable amounts. The UI explicitly separates intrinsic wage deductions from cash-flow payouts (advances), utilizing distinct color psychology (Green for net production, Red for payable debt) and interactive modals for deeper transparency.
- **Intelligent Viewport Management**: To enhance the administrative experience with extensive data, the UI employs CSS `position: sticky` for employee headers to remain visible during scroll. Furthermore, accordion toggle events dynamically calculate offsets and trigger smooth `window.scrollTo` adjustments, preventing jarring layout shifts when collapsing large time-punch histories.
