...
7. **Migration & Backup**: The legacy `handleBackupUpload` function was rewritten into a one-time Migration Script that parses a legacy `payroll_backup.json` and bulk-upserts all historical local data into the cloud database.

## 4. UI Architecture
- **Zero-Dependency Core**: Vanilla HTML/CSS/JS with the Supabase JS v2 client as the only external SDK.
- **Partial DOM Rendering**: Core changes via `timeEdit`, `cellEdit` and `resetRow` trigger `calculateAllData(true)` to recompute payroll math universally, but bypass the recursive `renderAll()` process. Instead, localized `innerHTML/outerHTML` manipulation applies diffs only to the targets. This prevents UX friction such as open accordions collapsing post-edit.
